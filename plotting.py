...
def plot_var(df, var: str, bins: np.ndarray,
    ...
    for this_df, this_scale in zip(df,scale):
        for i, entry in enumerate(signal_dict):
            ...
            hist, bin_edges = np.histogram(get_slices(this_df,this_signal_val)[var],bins=bins)
            binned_stats[:,df_counter] += hist
            counts[i] +=  len(get_evt(get_slices(this_df,this_signal_val)))*this_scale
...
def plot_var_pdg(df, var: str, bins: np.ndarray,
    ...
    hists   = np.zeros((len(pdg_dict),len(bin_steps)))
    steps   = np.zeros((len(pdg_dict),len(bins))) # for the step plot
    bottom  = np.zeros((len(pdg_dict),len(bins))) # for the bar plot 
    order   = np.arange(len(pdg_dict),0,step=-1)
    ...
    for idx in range(len(df)):
        ...
        for i, key in enumerate(list(pdg_dict.keys())): 
            ...
            if (key == "n" or key == "other" or key=='pi0'):
                continue
    ...
    for i in range(len(hists)): 
        ...
        if i==0: 
            ...
            steps[i] = np.insert(hists[i],obj=0,values=hists[i][0]) + steps[i-1]; 
            bottom[i] = steps[i-1]
        ax.step(bins,steps[i], label=plot_label, zorder=order[i],color=pdg_colors[i])
        if pdg_hatch == None: pdg_hatch = [""]*len(hists)
        ax.fill_between(bins, bottom[i], steps[i], step="pre", alpha=0.25, color=pdg_colors[i])
        ax.fill_between(bins, bottom[i], steps[i], step="pre", 
...
def plot_mc_data(mc_dfs, data_df, var, bins, 
    ...
    if pdg==False:
        ...
        mc_bins, mc_steps, mc_err = plot_var_pdg(df=mc_dfs, var=var, bins=bins, scale=scale,ax=ax_main, normalize=normalize)
